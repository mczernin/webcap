...
Section | Description
--- | ---
1. [Command line tool](#1-command-line-tool) | How to use the command line tool
2. [HTTP API server](#2-http-api-server) | How to start the server
3. [HTTP API](#3-http-api) | How to use the HTTP API
4. [Request Parameters](#4-request-parameters) | What parameters the APIs accept
5. [Returned Data](#5-returned-data) | What command line tool returns
6. [Examples](#6-examples) | Examples
7. [Heroku](#7-heroku) | How to run the HTTP API on Heroku
8. [License](#8-license) | UNLICENSE.txt (Public Domain)
...
`json` – See "[Request Parameters](#4-request-parameters)".
...
> If `'success'` then `r.data` is `json` — See "[Returned Data](#5-returned-data)".  
...
--- | --- | ---
`opts` | `object` | Options used during request (except `key`, `callbackUrl` and `cookies`). See "[Request Parameters](#4-request-parameters)".
...
Display screenshot using [ImageMagick]:

    $ phantomjs webcap.js '{"urls":[{"url":"http://fox.com"}]}' | grep -oP '(?<= "image": ").*(?=")' | base64 -d | display png:-

Display screenshot using [ImageMagick], using many parameters:
...
Simulate HTTP POST using [cURL] and display the resulting screenshot with [ImageMagick]:
...
See [`browser-example.js`][browserexample] for a Javascript example runnable inside a web browser.
  ...
  [browserexample]: https://github.com/gima/webcap/blob/master/documentation/browser-example.js
